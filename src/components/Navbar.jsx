...
import { assets } from "../assets/assets"; // Importing icons & images
  ...
  return (
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
      ...
      <div className="hidden sm:flex items-center gap-8">
        ...
        {/* If user not logged in → show Login button, else show profile dropdown */}
        {!user ? (
          <button
      ...
      {open && (
        ...
        >
          ...
          {/* Show Login or Logout in mobile */}
          {!user ? (
            <button
              onClick={() => {
                setOpen(false);
